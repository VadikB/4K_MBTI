...
Версия приложения увеличивается один раз при подготовке кандидата для
тестового контура, а не на каждый рабочий коммит. Командный Git-процесс,
правила Pull Request и продвижения между контурами описаны в
`CONTRIBUTING.md`.
...
После этого `git commit` проверяет формат staged diff и не позволяет
закоммитить изменения `web/js` без пересобранного `web/dist`.
...
В проекте используется `npm` и локальный `esbuild` из `node_modules`. `bun` для сборки не нужен.

Единый порядок для локального Макбука и сервера:
...
`npm install` нужен только после первого клонирования или изменения `package-lock.json`. После правок в `web/js` достаточно выполнить `npm run build:web`, чтобы обновить `web/dist/main.js` и связанные chunk-файлы.
...
ASSESSMENT_EXTERNAL_ANSWER_TRANSFER_ENABLED=false
...
Если суперадмин забыл пароль, сброс выполняется серверной командой. Скрипт работает только для email, который перечислен в `SUPERADMIN_EMAILS`; для остальных пользователей он откажется выполнять сброс.
